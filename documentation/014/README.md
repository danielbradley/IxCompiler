...
static void ParseParameters( Node* parent, Tokenizer* tokenizer );
static bool ParseParameter ( Node* parent, Tokenizer* tokenizer );
...
static void ParseMethod( Node* parent, Tokenizer* tokenizer )
{
    while ( Tokenizer_hasMoreTokens( tokenizer ) )
    {
        ...
        {
            ParseParameters( Node_getLastChild( parent ), tokenizer );
        }
        else
        if ( (token_group == OPEN) && (token_type == STARTBLOCK) )
        {
...
static void ParseParameters( Node* parent, Tokenizer* tokenizer )
{
    while ( Tokenizer_hasMoreTokens( tokenizer ) )
    {
        ...
        if ( token_type == WORD )
        {
            if ( ParseParameter( Node_getLastChild( parent ), tokenizer ) )
            {
                break;
            }
        }
...
static bool ParseParameter( Node* parent, Tokenizer* tokenizer )
{
    while ( Tokenizer_hasMoreTokens( tokenizer ) )
    {
        ...
        if ( (token_type == SYMBOL) && (',' == content[0]) )
        {
            return FALSE;
        }
        else
...
void IxSourceMethod_parseParameters  ( IxSourceMethod* self, const Node* startNode    );
```
...
IxSourceMethod* IxSourceMethod_new( const Node* modifierNode )
{
    ...
    if ( self )
    {
        ...
        self->parameters = Array_new_destructor( (void*(*)(void**)) IxSourceParameter_free );
...
void IxSourceMethod_parseNameChildren( IxSourceMethod* self, const Node* nameNode )
{
    ...
    if ( NodeIterator_hasNonWhitespaceOfType( it, STARTEXPRESSION ) )
    {
        ...
        IxSourceMethod_parseParameters( self, startNode );
    }
    else
    {
        self->invalid = TRUE;
...
IxSourceSignature* IxSourceSignature_new( String** access_modifier, bool const, String** method_name, String** return_type );
...
```c/ixcompiler.IxSourceSignature.c
IxSourceSignature* IxSourceSignature_new( String** access_modifier, bool is_const, String** method_name, String** return_type )
{
...
static void initChildren( IxSourceUnit* self, const AST* ast )
{
    ...
    self->methods        = Array_new_destructor( (void*(*)(void**)) IxSourceMethod_free );
    ...
    while ( NodeIterator_hasNonWhitespace( it ) )
    {
        ...
        {
            Array_push( self->methods,  Give( IxSourceMethod_new( node ) ) );
        }
    }
...
static String* GenerateMethods( const IxSourceUnitCollection* sourceUnits )
{
    ...
    String* methods = null;
    {
        StringBuffer* sb = StringBuffer_new();
        {
            ...
            {
                const IxSourceUnit* sourceUnit = IxSourceUnitCollection_get( sourceUnits, i );
                StringBuffer_appendLine( sb, "", GenerateMethodsForSourceUnit( sourceUnit, resolvedTypes ) );
            }
        ...
        StringBuffer* sb = StringBuffer_new();
        {
            const Array* methods = IxSourceUnit_getMethods( sourceUnit );
            {
                ...
                for ( int i=0; i < n; i++ )
                {
                    ...
                    StringBuffer_appendLine( sb, "", GenerateFunctionForMethod( method, resolvedTypes, classPrefix ) );
                }
            }
...
static String* GenerateFunctionForMethod( const IxSourceMethod* method, const Dictionary* resolvedTypes, const char* classPrefix )
{
    String* st_method = null;
    {
        ...
        StringBuffer* sb = StringBuffer_new();
        {
            ...
            const Array* parameters = IxSourceMethod_getParameters( method );
            if ( 0 < Array_getLength( parameters ) )
            {
                StringBuffer_appendLine( sb, "", GenerateParametersForMethod( method, resolvedTypes ) );
            }
...
static String* GenerateParametersForMethod( const IxSourceMethod* method, const Dictionary* resolvedTypes )
{
    String* st_parameters = null;
    {
        StringBuffer* sb = StringBuffer_new();
        {
            ...
            for ( int i=0; i < n; i++ )
            {
                const IxSourceParameter* parameter = (IxSourceParameter*) Array_getObject( parameters, i );
                const IxSourceType*      type      = IxSourceParameter_getType( parameter );
                const String*            name      = IxSourceParameter_getName( parameter );
                const String*            typeName  = IxSourceType_getName( type );

                if ( !IxSourceType_isFullName( type ) && Dictionary_get( resolvedTypes, typeName ) )
                {
                    typeName = Dictionary_get( resolvedTypes, typeName );
                }

                {
                    String* ctype = String_replace( typeName, '.', '_' );
